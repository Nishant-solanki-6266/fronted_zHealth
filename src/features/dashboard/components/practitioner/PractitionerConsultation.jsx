...
import BodyChartDiagram from './BodyChartDiagram'
...
export default function PractitionerConsultation() {
  ...
  useEffect(() => {
    ...
    if (apptObj) {
      ...
      if (existingNoteObj) {
        setNoteText(existingNoteObj.notes || '')
        if (existingNoteObj.soap && typeof existingNoteObj.soap === 'object') {
          if (Array.isArray(existingNoteObj.soap.bodyChart)) {
            setBodyChartFindings(existingNoteObj.soap.bodyChart)
          }
          soapForm.setFieldsValue(existingNoteObj.soap)
        }
            ...
            <div>
              ...
              <div className="flex items-center gap-3 mt-1.5">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">
                  Funding: <span className="text-[#8C4BFF]">{patient.fundingScheme || 'Private'}</span>
                </span>
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-wider">
                  Sessions Remaining: <span className={patient.sessionsAllocated - patient.sessionsUsed <= 2 ? 'text-red-500' : 'text-[#10B981]'}>{patient.sessionsAllocated - patient.sessionsUsed}</span>
                </span>
              </div>
            </div>
          </div>
