...
function renderSectionContent(section, role, store, navigate, modalContextProps) {
  ...
  if (role === 'practitioner') {
    switch (section) {
      ...
      case 'notes-reports':
        return <PractitionerNotesReports />
      case 'exercises-treatment-plans':
        return <PractitionerExercisesPlans />
      case 'referrals':
        return <PractitionerReferrals />
      case 'billing':
        return <PractitionerBilling />
      case 'messages':
        return <PractitionerMessages />
      case 'tasks':
        return <PractitionerTasks />
  ...
  if (role === 'patient') {
    switch (section) {
      ...
      case 'progress':
        return <PatientProgressOutcomes />
      case 'forms-documents':
        return <PatientFormsDocuments />
      case 'funding':
        return <PatientFundingClaims />
      case 'payments':
        return <PatientPayments />
      case 'messages':
        return <PatientMessages />
      case 'health-sharing':
        return <PatientHealthSharing />
      default:
        break
    }
  }
...
export default function DashboardPage() {
      ...
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
            <Link to="/clinic" className="hover:text-[#8C4BFF] transition-colors">Dashboard</Link>
  ...
  return (
    <div className="space-y-6">
      {userRole === 'head_admin' && <HeadAdminDashboard store={store} navigate={navigate} />}
      {userRole === 'sales' && <SalesDashboard store={store} navigate={navigate} modalContext={modalContextProps} />}
      {userRole === 'practitioner' && <PractitionerDashboard store={store} navigate={navigate} />}
      {userRole === 'patient' && <PatientDashboard store={store} navigate={navigate} />}
      {userRole === 'clinic' && <ClinicAdminDashboard store={store} />}
    </div>
  )
}
