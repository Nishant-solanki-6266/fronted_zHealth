...
export default function ClinicAdminCalendarPage() {
  ...
  const getApptsInSlot = (dayObj, h, m) => {
    ...
    return dayAppts.filter(a => {
      const timeStr = a.time || a.startTime
      if (!timeStr || typeof timeStr !== 'string') return false
      const parts = timeStr.split(':')
      if (parts.length < 2) return false
      const ah = Number(parts[0])
    ...
    <div className="flex flex-col" style={{ height: 'calc(100vh - 96px)' }}>
      ...
      <div className="flex items-center justify-between pb-4 gap-3 flex-wrap">
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3">
          <h2 className="text-lg md:text-xl font-extrabold text-slate-800 min-w-[130px] dark:text-white">
