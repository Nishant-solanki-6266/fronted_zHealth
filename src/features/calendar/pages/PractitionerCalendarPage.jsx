    ...
    if (patientId && (newAppointment || rescheduleApptId)) {
      ...
      if (newAppointment) {
        ...
        if (!hasShownPopup.current) {
          Modal.info({
            title: `Booking Appointment for ${pName}`,
            content: `Please click on any available slot in the calendar to book an appointment for ${pName}.`,
            okText: 'Got it',
            centered: true,
            icon: <CalendarOutlined style={{ color: '#8C4BFF' }} />,
            okButtonProps: { style: { backgroundColor: '#8C4BFF', borderColor: '#8C4BFF', borderRadius: 8 } }
          });
        ...
        if (!hasShownPopup.current) {
          Modal.info({
            title: `Rescheduling Appointment for ${pName}`,
            content: `Please click on any available slot in the calendar to reschedule the appointment for ${pName}.`,
            okText: 'Got it',
            centered: true,
            icon: <CalendarOutlined style={{ color: '#8C4BFF' }} />,
            okButtonProps: { style: { backgroundColor: '#8C4BFF', borderColor: '#8C4BFF', borderRadius: 8 } }
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 96px)' }}>
      ...
      <div className="flex-grow flex flex-col lg:flex-row gap-4 overflow-hidden h-full">
        {showMiniCalendar && (
          <div className="w-full lg:w-64 flex-shrink-0 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-4 rounded-2xl shadow-sm flex flex-col lg:h-full overflow-y-auto max-h-[280px] lg:max-h-none">
